  ...
  return (
    ...
    >
          ...
          {['HOME', 'CONTACT US', 'BLOG'].map((item, index) => (
            ...
            >
              <Link 
                ...
                className={`relative font-medium px-5 py-2 transition-all ${
                  scrolled ? 'text-gray-700 hover:text-indigo-700' : 'text-gray-100 hover:text-white'
                }`}
              >
                <motion.span 
          ...
          >
            <Link href="#">
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.98 }}
                className={`font-medium px-6 py-2 rounded-full shadow-lg transition-all ${
                  scrolled 
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div 
            className="md:hidden absolute top-full left-0 right-0 w-full bg-gradient-to-b from-indigo-900 to-purple-900 backdrop-blur-md"
